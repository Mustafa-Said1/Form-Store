...
form.addEventListener("submit", function (e) {
    ...
    if (!fnameTest) {
        fnameField.nextElementSibling.innerHTML = `
        <li class="invalid-text">Username is required and must be valid!</li>
        `;
    }
    if (!emailTest) {
        emailField.nextElementSibling.innerHTML = `
        <li class="invalid-text">Email is required and must end with @gmail.com!</li>
        `;
    }
    if (!phoneTest) {
        phoneField.nextElementSibling.innerHTML = `
        <li class="invalid-text">Phone number is required (11 digits starting with 010/011/012/015)!</li>
        `;
    }
    if (!passwordTest) {
        passwordField.nextElementSibling.innerHTML = `
        <li class="invalid-text">Password is required (Minimum 8 letters or numbers)!</li>
        `;
    }
